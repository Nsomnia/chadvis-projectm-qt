...
### Suno Integration
- [~] **P1: Suno Core Correctness** — secure credential storage, lossless asynchronous restore readiness for Library/Explore, queued authenticated requests, feed/clip/account parsing, same-host `tokens` fallback, and method-specific Studio headers are implemented. Route preference/fallback order, Settings credential ownership, sign-out, and authorized runtime behavior remain `[VERIFY]`.
...
- [~] **Fail closed on unverified routes and hosts** — active Orpheus/Modal, WAV conversion, legacy Clerk-host fallback, constructed-media, and user-reachable lead routes are disabled; QML artwork is restricted to the exact captured Suno CDN origins. Declaration-only fetch methods, canonical Clerk session selection, and route-specific header policy still need retirement or capture-backed implementation. Never send cookies/bearers or automatic remote image requests to unverified absolute hosts.
