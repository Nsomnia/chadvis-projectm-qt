...
### Audio engine
- [ ] **Shuffle with Repeat::Off skips a track** — `setShuffle` leaves `shufflePosition_ = 0` while `next()` pre-increments it, so `shuffleOrder_[0]` is never played: 3 items, only 2 visited. Separately, `addFile` Fisher-Yates-swaps the new index into `shuffleOrder_` but `addUrl` does not, so the two entry points build different shuffle orders. Navigation semantics were deliberately **not** changed by the thread-safety refactor; both cases are pinned in `test_Playlist.cpp` with comments so a future fix is a visible decision rather than a silent behaviour change.
- [ ] **`spectrumUpdated` has zero consumers** — verified: `VisualizerBridge` contains no reference to `AudioEngine` at all, and only `stateChanged`/`positionChanged`/`durationChanged`/`trackChanged` are connected (LyricsSync, AudioBridge). So `AudioEngine::currentSpectrum_` is assigned once per analyzer tick and copied into a 1 KB `QAudioSpectrum` argument for no reader. Same dead-plumbing class as `pcmReceived` (removed) but unlike it, this one still has an emit site — delete the signal and the member, or wire it to the visualizer. `onPlaylistCurrentChanged` (`AudioEngine.cpp:138-142`) calls `play()` **unconditionally** at `:141`, and that handler is bound to `currentChanged` at `:45`, i.e. to *every* index change including user-initiated jumps and playlist edits while paused. `autoPlayNext_` guards only the `EndOfMedia` path, so nothing guards the manual path. Small, contained, user-facing.
