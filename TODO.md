...
- [x] **Fail-closed requests** — active Orpheus/Modal, WAV conversion, constructed-media, user-reachable lead routes, automatic unverified `/tokens` fallback, noncanonical session selection, undocumented Browser-Token synthesis, and declaration-only fetch declarations/signals are removed or disabled; authorized runtime validation remains.
