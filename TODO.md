...
- [~] **Fail-closed requests** — active Orpheus/Modal, WAV conversion, constructed-media, user-reachable lead routes, automatic unverified `/tokens` fallback, and noncanonical session selection are disabled; declaration-only fetch methods and authorized runtime validation remain to be retired or completed.
